...
export default function LeaveApprovalDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`http://40.75.121.168:8089/api/leaves/manager/${managerId}`, {
          method:'GET',
          headers:{
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token')
      await axios.put(`http://40.75.121.168:8089/api/leaves/approve/${id}`, null,  {
  ...
  const handleReject = async () => {
    ...
    const token = localStorage.getItem('token')
      await axios.put(`http://40.75.121.168:8089/api/leaves/reject/${selectedLeaveId}/${rejectionReason}`, null,  {
